        ...
        <div className="md:col-span-3">
          ...
          <div>
            <div className="text-xs flex items-center gap-5 my-7">
              ...
              <p className="flex items-center gap-1 text-lightColor relative group hover:cursor-pointer hover:text-shop-dark-green hoverEffect">
                <Calendar size={15} />{" "}
                {blog?.publishedAt ? dayjs(blog.publishedAt).format("MMMM D, YYYY") : "No date"}
                <span className="absolute left-0 -bottom-1.5 bg-lightColor/30 inline-block w-full h-0.5 group-hover:bg-shop-dark-green hoverEffect" />
              </p>
            </div>
          ...
          {categories?.map(({ blogcategories }, index) => (
            <div key={index}>
              {blogcategories && blogcategories.length > 0 && (
                <div className="text-lightColor flex items-center justify-between text-sm font-medium">
            ...
            >
              {blog?.mainImage && (
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-16 h-16 rounded-full object-cover border border-shop-dark-green/10 group-hover:border-shop-dark-green hoverEffect"
                />
              )}
